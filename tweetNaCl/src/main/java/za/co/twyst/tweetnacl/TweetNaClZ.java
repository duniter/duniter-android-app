 ...
 * implementation. 
 * <p>
 * The TweetNaClZ is an experimental 'higher performance' implementation 
 * that differs from the standard implementation:
 * <ul>
 * <li> the invoking method is expected to manage and byte array allocation and 
 *      initialisation
 * <li> does no validation of input
 * <li> does no memory zero'ing and other cleanup
 * </ul>
 * 
...
public class TweetNaClZ {
    ...
    public byte[] cryptoBox(final byte[] message, final byte[] nonce, byte[] publicKey, byte[] secretKey) throws EncryptException {
        ...
        byte[] ciphertext = new byte[message.length - BOX_BOXZEROBYTES];
        int    rc;
        
        if ((rc = jniCryptoBox(c, message, nonce, publicKey, secretKey)) != 0) {
