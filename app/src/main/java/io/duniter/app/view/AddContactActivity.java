...
public class AddContactActivity extends ActionBarActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        currencyId = getIntent().getLongExtra(Application.CURRENCY_ID,-1);
...
//        });
    ...
    public void send(Contact entity, String message) {
        if(entity!=null) {
            ...
            this.currency = SqlService.getCurrencySql(this).getById(entity.getCurrency().getId());
        }
    }
...
//        }
    ...
    private void actionAddContact() {
        ...
        contact.setUid(uid.isEmpty() ? "" : uid);
        contact.setAlias(name.isEmpty() ? contact.getUid() : name);
        contact.setCurrency(currency);
        contact.setPublicKey(publicKey);

        contact.setId(SqlService.getContactSql(this).insert(contact));
        askContactInPhone();
    }
    ...
    private void addNewContactInPhone(){
        ...
        String url = Format.createUri(Format.LONG, uid, publicKey, currency.getName());
